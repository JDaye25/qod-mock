...
def require_api_key(
    ...
    Behavior (STRICT):
      - If QOD_API_TOKEN is set (non-empty): header is REQUIRED.
        * Missing header -> 401
        * Wrong scheme -> 401
        * Wrong token -> 401
        * Correct token -> returns token string

      - If QOD_API_TOKEN is NOT set: server is misconfigured for "auth required"
        (fail closed) -> 500

    NOTE: We read QOD_API_TOKEN at request-time (NOT import-time) so it works
    reliably with uvicorn --reload / multi-process reloaders.
    """
    expected = (os.getenv("QOD_API_TOKEN") or "").strip()

    # Fail closed so you never accidentally run "auth off" and think you're protected.
    if not expected:
        raise HTTPException(
...
def _debug_env() -> Dict[str, Any]:
    ...
    auth_required = True  # because require_api_key fails closed if token missing
    return {
...
# Use decorator-level dependencies so protection cannot be "accidentally not applied"
# ----------------------------
@app.post("/intent", dependencies=[Depends(require_api_key)])
...
@app.post("/proof/{session_id}/finalize", dependencies=[Depends(require_api_key)])
...
@app.get("/proof/{session_id}", dependencies=[Depends(require_api_key)])
...
@app.get("/proof/{session_id}/bundle", dependencies=[Depends(require_api_key)])
...
@app.get("/proof/{session_id}/verify", dependencies=[Depends(require_api_key)])
